...
begin
  ...
  for cleanup_table, cleanup_ids in
    select *
    from (
      values
        ...
        ('child_attendance_monthly', array[
    ...
    ) as cleanup(cleanup_table, cleanup_ids)
  loop
    if to_regclass(format('public.%I', cleanup_table)) is not null then
      execute format('delete from public.%I where id = any ($1)', cleanup_table)
      using cleanup_ids;
    end if;
  end loop;
...
begin
  ...
  if exists (
    ...
    return jsonb_build_object('seededExtra', false, 'skippedMissingTables', true);
  end if;

...
export default function DiagnosticExplorer({ childId }: Props) {
  ...
  if (engine.isProcessing || engine.phase === 'processing') {
    return (
      <LinearGradient colors={['#1A1040', '#2D1B69', '#6C5CE7']} style={styles.fullScreen}>
        <SafeAreaView style={styles.centered}>
          ...
          <Text style={styles.processingSubtitle}>Создаём персональный профиль талантов</Text>
        </SafeAreaView>
      </LinearGradient>
    );
  }
  ...
  if (engine.phase === 'intro') {
    return (
      <LinearGradient colors={['#6C5CE7', '#AF52DE', '#8B7FE8']} style={styles.fullScreen}>
        <SafeAreaView style={styles.centered}>
          <MotiView
    ...
    >
      ...
      >
        <MotiView
          ...
          style={[styles.progressFill, isProPhase && { backgroundColor: '#A78BFA' }]}
        />
      </View>
